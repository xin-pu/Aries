...
namespace Aries.Core
{
    public static class FileServiceProviderWpf
    {
        ...
        public static void SerializeGraphDataToFile(string filename, GraphCVFile modelsList)
        {
        ...
        /// </summary>
        /// <param name="filename">File name</param>
        public static GraphCVFile DeserializeGraphDataFromFile(string filename)
        {
        ...
        /// <param name="stream">The destination stream</param>
        /// <param name="modelsList">The graph data</param>
        public static void SerializeDataToStream(Stream stream, GraphCVFile modelsList)
        {
            var serializer = new YAXSerializer(typeof(GraphCVFile));
            using var textWriter = new StreamWriter(stream);
            serializer.Serialize(modelsList, textWriter);
            textWriter.Flush();
        }
        ...
        /// <param name="stream">The stream</param>
        /// <returns>The graph data</returns>
        public static GraphCVFile DeserializeGraphDataFromStream(Stream stream)
        {
            var deserializer = new YAXSerializer(typeof(GraphCVFile));
            using var textReader = new StreamReader(stream);
            return (GraphCVFile)deserializer.Deserialize(textReader);
        }
    }
}
