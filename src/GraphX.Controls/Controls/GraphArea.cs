...
namespace GraphX.Controls
{
        ...
        where TGraph : class, IMutableBidirectionalGraph<TVertex, TEdge>
    {
        ...
        /// <summary>
        ///     Gets or sets vertex label control factory. Vertex labels will be generated at the end of the graph generation
        ///     process.
        /// </summary>
        ...
        /// <summary>
        ///     Gets or sets control factory class that allows you to define what vertex and edge controls will be generated by
        ///     GraphX
        /// </summary>
        ...
        /// <summary>
        ///     Gets or sets if visual properties such as edge dash style or vertex shape should be automaticaly reapplied to
        ///     visuals when graph is regenerated.
        ///     True by default.
        /// </summary>
        public bool EnableVisualPropsRecovery { get; set; }

        /// <summary>
        ///     Gets or sets if visual properties such as edge dash style or vertex shape should be automaticaly applied to newly
        ///     added visuals which are added using AddVertex() or AddEdge() or similar methods.
        ///     True by default.
        /// </summary>
        ...
        private void RemoveVertexInternal(VertexControl ctrl, bool removeVertexFromDataGraph = false)
        {
            ...
            if (removeVertexFromDataGraph && LogicCore?.Graph != null &&
                LogicCore.Graph.ContainsVertex(ctrl.Vertex as TVertex))
        ...
        protected override void RemoveAnimatedControl(IGraphControl ctrl, bool removeDataObject)
        {
            ...
            if (edgeControl != null) RemoveEdgeInternal(edgeControl, removeDataObject);
        }
        ...
        ///     Add data vertex to graph and vertex control to layout. LogicCore::Graph should be assigned or exception will be
        ///     thrown.
        /// </summary>
        /// <param name="vertexData">Vertex data object</param>
        /// <param name="vertexControl">Vertex visual control object</param>
        ...
        /// <summary>
        ///     Insert an edge to layout at specified position. By default, edge is inserted into the begining of the visual tree
        ///     causing it to be rendered below all of the vertices.
        /// </summary>
        /// <param name="edgeData">Edge data object</param>
        /// <param name="edgeControl">Edge visual control</param>
        ...
        /// <summary>
        ///     Usability extension method.
        ///     Insert an edge to layout at specified position and add data edge. By default, edge is inserted into the begining of
        ///     the visual tree causing it to be rendered below all of the vertices.
        ...
        protected virtual int GetNextUniqueId(bool isVertex)
        {
            if (isVertex)
            {
                while (DataIdsCollection.Contains(_dataIdCounter)) _dataIdCounter++;
                DataIdsCollection.Add(_dataIdCounter);
                return _dataIdCounter;
            }

            while (EdgeDataIdsCollection.Contains(_edgeDataIdCounter)) _edgeDataIdCounter++;
        ...
        public Dictionary<TVertex, Size> GetVertexSizesAndPositions(out IDictionary<TVertex, GPoint> vertexPositions)
        {
            //measure if needed and get all vertex sizes
            Measure(new USize(double.PositiveInfinity, double.PositiveInfinity));
            ...
            vertexPositions = new Dictionary<TVertex, GPoint>(count);
            //go through the vertex presenters and get the actual layoutpositions
            foreach (var vc in VertexList.Where(vc =>
                ((IGraphXVertex) vc.Value.Vertex).SkipProcessing != ProcessingOptionEnum.Exclude))
            {
        ...
        /// </summary>
        public Dictionary<TVertex, GPoint> GetVertexPositions()
        {
            return VertexList
        ...
        ///     Generates visual objects for all vertices and edges w/o any algorithms. Objects are hidden by default. Optionaly,
        ...
        /// <param name="autoresolveIds">
        ///     Automaticaly assign unique Ids to data objects. Can be vital for different GraphX logic
        ///     parts such as parallel edges.
        /// </param>
        ...
        /// <summary>
        ///     Clears all visual objects and generates VertexControl objects from specified graph. All vertices are created hidden
        ///     by default.
        ...
        protected virtual void _relayoutGraph(CancellationToken cancellationToken)
        {
            ...
            IDictionary<TVertex, GPoint> vertexPositions = null;
        ...
        /// <param name="generateAllEdges">Generate all available edges for graph</param>
        /// <param name="dataContextToDataItem">
        ///     Sets visual edge and vertex controls DataContext property to vertex data item of
        ///     the control (Allows prop binding in xaml templates)
        /// </param>
        ...
        /// <param name="updateEdgesOnMove">
        ///     Is edges update enabled while dragging (use this if you have edge routing algorithms
        ///     enabled)
        /// </param>
        ...
        public void SetEdgesDrag(bool isEnabled)
        {
            ...
            foreach (var item in EdgesList) DragBehaviour.SetIsDragEnabled(item.Value, isEnabled);
        }
        ...
        internal override void ComputeEdgeRoutesByVertex(VertexControl vc, bool vertexDataNeedUpdate = true)
        {
            ...
            LogicCore.ComputeEdgeRoutesByVertex((TVertex) vc.Vertex,
                vertexDataNeedUpdate ? (GPoint?) vc.GetPositionGraphX() : null,
                vertexDataNeedUpdate ? (Size?) new Size(vc.ActualWidth, vc.ActualHeight) : null);
        }
        ...
        protected virtual void GenerateAllEdgesInternal(Visibility defaultVisibility = Visibility.Visible)
        {
            ...
            foreach (var item in _edgeslist) item.Value.PrepareEdgePath();
        ...
        ///     Ensures that layout is properly updated before edges calculation. If you are sure that it is
        ///     already updated you can set this param to False to increase performance.
        /// </param>
        ...
        /// <summary>
        ///     Update parallel edges information. Only needed to be run when edges has been added manualy and has to track
        ///     parallel ones.
        ///     Essentialy refreshes EdgeControl::IsParallel property
        /// </summary>
        /// <param name="edgeList">
        ///     Optonal parameter. Specifies initial list of edges. If null then all edges are parsed. Default
        ///     value is Null.
        /// </param>
        ...
        /// </summary>
        /// <param name="performFullUpdate">
        ///     If True - perform full edge update including all children checks such as pointers &
        ///     labels. If False - update only edge routing and edge visual
        /// </param>
            ...
            EdgesType edgesType = EdgesType.All)
        {
            ...
            if (edge.Target != null && _vertexlist.ContainsKey(edge.Target)) list.Add(_vertexlist[edge.Target]);
            if (edge.Source != null && _vertexlist.ContainsKey(edge.Source)) list.Add(_vertexlist[edge.Source]);
        ...
        {
            IDictionary<TVertex, GPoint> vPositions;
        ...
        /// <param name="useZoomControlSurface">
        ///     Use zoom control parent surface to render bitmap (only visible zoom content will be
        ///     exported)
        /// </param>
            ...
            double dpi = PrintHelper.DEFAULT_DPI, int quality = 100)
        {
            ...
            if (dlg.ShowDialog() == true) ExportAsImage(dlg.FileName, itype, useZoomControlSurface, dpi, quality);
#endif
        }
...
#endif
        /// <summary>
        ///     Sets GraphArea into printing mode when its size will be recalculated on each measuer and child controls will be
            ...
            bool clearLogicCore = false)
        {
            ...
            if (clearLogicCore || clearStates) _stateStorage?.Dispose();
        }
        ...
        /// </summary>
        /// <param name="control">Visual entity</param>
        /// <param name="moveLabels">
        ///     Also move attached labels (if any). Disable to decrease performance hit if you don't use
        ///     labels.
        /// </param>
